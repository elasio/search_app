...
def download_test(data_dir):
    ...
    for di in range(len(datasets)):
        ...
        if not os.path.isdir(dst_dir):
            ...
            if dataset == 'oxford5k' or dataset == 'paris6k':
                ...
                for dli in range(len(dl_files)):
                    ...
                    os.system('wget {} -O /content/drive/My\ Drive/Image_retrieval/cnnimageretrieval-pytorch/data/test/oxford5k/jpg/oxbuild_images.tgz'.format(src_file))
                    print('>> Extracting dataset {} archive {}...'.format(dataset, dl_file))
                    # create tmp folder
                    dst_dir_tmp = os.path.join(dst_dir, 'tmp')
                    os.system('mkdir /content/drive/My\ Drive/Image_retrieval/cnnimageretrieval-pytorch/data/test/oxford5k/jpg/tmp')
                    # extract in tmp folder
                    os.system('tar -zxf /content/drive/My\ Drive/Image_retrieval/cnnimageretrieval-pytorch/data/test/oxford5k/jpg/oxbuild_images.tgz -C /content/drive/My\ Drive/Image_retrieval/cnnimageretrieval-pytorch/data/test/oxford5k/jpg/tmp/')
                    # remove all (possible) subfolders by moving only files in dst_dir

                    temp_1 = '/content/drive/My\ Drive/Image_retrieval/cnnimageretrieval-pytorch/data/test/oxford5k/jpg/tmp'
                    temp_2 = '/content/drive/My\ Drive/Image_retrieval/cnnimageretrieval-pytorch/data/test/oxford5k/jpg'
                    os.system('find {} -type f -exec mv -i {{}} {} \\;'.format(temp_1, temp_2))
                    # remove tmp folder
                    os.system('rm -rf {}'.format(temp_1))
                    print('>> Extracted, deleting dataset {} archive {}...'.format(dataset, dl_file))
                    os.system('rm /content/drive/My\ Drive/Image_retrieval/cnnimageretrieval-pytorch/data/test/oxford5k/jpg/oxbuild_images.tgz')
        ...
        if not os.path.exists(gnd_dst_file):
            ...
            os.system('wget {} -O {}'.format(gnd_src_file, '/content/drive/My\ Drive/Image_retrieval/cnnimageretrieval-pytorch/data/test/oxford5k/' + gnd_dl_file))
...
def download_train(data_dir):
    ...
    if not os.path.isdir(dst_dir):
        ...
        os.system('wget {} -O {}'.format(src_file, process_path(dst_file)))
        print('>> Extracting {}...'.format(dst_file))
        os.system('tar -zxf {} -C {}'.format(dst_file, process_path(dst_dir)))
        print('>> Extracted, deleting {}...'.format(dst_file))
        os.system('rm {}'.format(process_path(dst_file)))
    ...
    if not os.path.isdir(dst_dir):
        ...
        os.system('ln -s {} {}'.format(dst_dir_old, process_path(dst_dir)))
    ...
    for dataset in datasets:
        ...
        for i in range(len(dl_files)):
            ...
            if not os.path.isfile(dst_file):
                ...
                temp = dst_file.split(' ')
                temp2 = temp[0] + '\ ' + temp[1]
                os.system('wget {} -O {}'.format(src_file, temp2))
